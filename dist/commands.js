import { applyStash, checkoutBranch, deleteLocalBranch, getBranchList, getStashList, } from "./git.js";
...
const switchCommand = {
    name: "switch",
    execute: async (...args) => {
        const { branches, currentBranch } = await renderLoadingUntilComplete(getBranchList);
...
const addCommand = {
    name: "add",
    execute: async (...args) => {
        const { branches, currentBranch } = await renderLoadingUntilComplete(() => getBranchList(true));
...
export const Commands = new Map(commandList.map((command) => [command.name, command]));
